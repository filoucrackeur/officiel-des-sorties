...
# "middleware" invocation mechanism: traverse an async callback chain
# `final_cb` is a function receiving the parameters (err, req, res)
# `callbacks` is an array of callbacks
#   each callback is a function receiving (req, res, next)
#   if a callback accepts 4 parameters, (err, req, res, next), it's an
#   error handler
invoke_callbacks = (context, callbacks, req, res, final_cb) ->
  callback_invoke = (context, callbacks, i, err, req, res, final_cb) ->
    l = callbacks.length
    if err
      # an error happened, look for an error handler in the callback chain,
      # skipping regular callback invocation
      while i < l
        cb = callbacks[i]
        if cb.length >= 4
          # error handler - (err, req, res, next)
          next = (err) ->
            return final_cb.call(context, err, req, res)
          return cb.call(context, err, req, res, next)
      # no error handler found, use default strategy
      console.log("unhandled error in express-mongoose-resource callback invocation:", err)
      return final_cb.call(context, err, req, res)

    # no error, continue traversing the callback chain

    if i >= l
      # the callback chain is terminated, call the final callback
      return final_cb.call(context, err, req, res)
    ...
    if cb.length >= 4
      ...
      return cb.call(context, err, req, res, next)
    # regular callback
    return cb.call(context, req, res, next)

  callback_invoke(context, callbacks, 0, null, req, res, final_cb)
...
class ModelController
  ...
  renderTemplate: (res, name, data, context) ->
    ...
    if @opts.render_cb?[view]?
      @opts.render_cb[view].call @, data, ctxt, (ctxt) =>
  ...
  preprocess_instance: (instance) ->
    ...
    invoke_callbacks @, @_pre_serialize_cbs, req, res, (err, req, res) =>
      return {}  if (err)

      req.object = @_serialize_cb(req.instance, req.toObject)

      invoke_callbacks @, @_post_serialize_cbs, req, res, (err, req, res) =>
        return {}  if (err)
        return req.object
